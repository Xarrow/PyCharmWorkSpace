...
若要设置引用属性的值，请将属性的值设置为实例对象路径用于标识现有的类实例。例如，图 23 列出的 MOF 语句可以将 NewRef 的属性设置为带有Name 属性的 ExistingClass 类的实例，这等同于直接使用 SomeName 对 ExistingClass 类实例的 Name属性进行赋值。
